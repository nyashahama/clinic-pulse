...
import type { WalkthroughLead, WorkspaceState } from "@/lib/workspace/types";
import {
...
type LeadStatusCount = Record<WalkthroughLead["status"], number>;

function buildLeadStatusCounts(leads: WalkthroughLead[]): LeadStatusCount {
  return leads.reduce(
    (accumulator, lead) => {
...
function getLatestAdminInteractionAt(state: WorkspaceState) {
  const timestamps = [
