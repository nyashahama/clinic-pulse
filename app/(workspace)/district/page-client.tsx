...
import { AlertList } from "@/components/workspace/alert-list";
import { ClinicMap } from "@/components/workspace/clinic-map";
import { ClinicTable } from "@/components/workspace/clinic-table";
import { DistrictCommandBrief } from "@/components/workspace/command-center/district-command-brief";
import { DistrictHome } from "@/components/workspace/command-center/district-home";
import { InterventionRail } from "@/components/workspace/command-center/intervention-rail";
import { SeverityQueue } from "@/components/workspace/command-center/severity-queue";
import { SignalAnalytics } from "@/components/workspace/command-center/signal-analytics";
import { SupportingOperations } from "@/components/workspace/command-center/supporting-operations";
import { VerificationHandover } from "@/components/workspace/command-center/verification-handover";
import { ScenarioControls } from "@/components/workspace/scenario-controls";
import { IncidentReplayPanel } from "@/components/workspace/incident-replay-panel";
import { PilotReadinessPanel } from "@/components/workspace/pilot-readiness-panel";
import { ReferenceSectionCards } from "@/components/workspace/reference-section-cards";
import { ReportStream } from "@/components/workspace/report-stream";
import { StatusSummary } from "@/components/workspace/status-summary";
...
import { buildDistrictHomeViewModel } from "@/lib/workspace/district-home-view-model";
import {
...
function buildClinicDataTrustState(
  ...
  return buildDataTrustState({
    source: pendingReviewClinicIds.has(clinic.id) ? "field_report" : "scenario_seed",
...
export default function DistrictConsolePage({
  ...
  const latestWorkspaceStateRef = useRef(state);
  ...
  const runIncidentReplayStep = (stepIndex: number, sessionId: number) => {
    ...
    if (step.id === "partner_webhook") {
      setWebhookPreview(
        buildIncidentReplayWebhookPreview(latestWorkspaceStateRef.current, stepNow),
      );
    }
