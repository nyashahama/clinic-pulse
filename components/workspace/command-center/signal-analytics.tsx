import type { DistrictCommandCenter, DistrictSeverityReasonCode } from "@/lib/workspace/district-command-center";
    ...
    >
      ...
      <div className="mt-4 rounded-2xl border border-border bg-muted p-4">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Top severity drivers</p>
        ...
        {analytics.topReasonCodes.length === 0 ? (
          <p className="mt-3 text-sm leading-6 text-muted-foreground">No severity drivers are currently affecting the queue.</p>
        ) : (
          <ol className="mt-3 grid gap-2">
            {analytics.topReasonCodes.map((reason, index) => (
              <li key={reason.code} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-3 py-2 text-sm">
