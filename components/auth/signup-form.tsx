...
function FieldLabel({
  ...
  return (
    <label htmlFor={htmlFor} className="mb-1.5 block text-xs font-medium text-neutral-600 dark:text-white/50">
      {children}
    </label>
  );
}
  ...
  return (
    <form action={formAction} className="grid gap-4">
      ...
      {state.error ? (
        <p
          role="alert"
          className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs font-medium text-amber-300"
        >
          {state.error}
        </p>
      ) : null}
