...
function EmailOnlySignIn() {
  ...
  return (
    ...
    >
      <label>
        <span className="mb-1.5 block text-xs font-medium text-neutral-600 dark:text-muted-foreground">
          Email address
        </span>
        <input
          id="email"
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3.5">
      ...
      {state.error ? (
        <p
          role="alert"
          className="rounded-lg border border-red-200/60 bg-red-50/50 px-3 py-2 text-xs font-medium text-red-700 dark:border-red-500/15 dark:bg-red-500/5 dark:text-red-300"
        >
          {state.error}
        </p>
      ) : null}
