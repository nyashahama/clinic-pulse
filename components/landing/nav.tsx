        ...
        <div className="relative mx-auto w-full max-w-screen-xl px-3 lg:px-10">
          <div className="flex h-14 items-center justify-between gap-3">
            ...
            <div className="flex grow basis-0 justify-end gap-2 lg:hidden">
              <Link
                href="/login"
                className="flex h-8 items-center rounded-lg border border-neutral-300 bg-white px-3 text-xs font-semibold text-neutral-900 transition-colors hover:bg-neutral-50"
              >
                Sign in
              </Link>
              <Link
                href="/?booking=1"
                className="flex h-8 items-center rounded-lg border border-neutral-900 bg-neutral-900 px-3 text-xs font-semibold text-white transition-colors hover:bg-neutral-800"
              >
                Book demo
              </Link>
            </div>
          ...
          <nav className="-mx-3 flex gap-1 overflow-x-auto border-t border-neutral-200 px-3 py-2 lg:hidden">
            {NAV_ITEMS.map(({ name, href }) => (
              <Link
                key={name}
                href={href}
                className="flex h-8 shrink-0 items-center rounded-md px-3 text-xs font-semibold text-neutral-600 transition-colors hover:bg-neutral-900/5 hover:text-neutral-900"
              >
                {name}
              </Link>
            ))}
            <span className="ml-1 inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md px-3 text-xs font-semibold text-neutral-500">
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-pulse-dot absolute inline-flex h-full w-full rounded-full bg-green-500 opacity-75" />
                <span className="inline-flex h-1.5 w-1.5 rounded-full bg-green-500" />
              </span>
