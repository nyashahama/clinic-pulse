...
const workspaceStoreSourcePath = path.join(process.cwd(), "lib", "workspace", "workspace-store.tsx");
...
describe("workspace store incident replay wiring", () => {
  it("exposes the replay store method, action type, and reducer wiring", () => {
    const workspaceStoreSource = readFileSync(workspaceStoreSourcePath, "utf8");

    expect(workspaceStoreSource).toContain(
      "applyIncidentReplayStep: (stepId: IncidentReplayStepId, now?: string) => void;",
    );
    expect(workspaceStoreSource).toContain('type: "apply_incident_replay_step"');
    expect(workspaceStoreSource).toContain('case "apply_incident_replay_step":');
    expect(workspaceStoreSource).toContain(
      "return applyIncidentReplayStep(state, action.stepId, action.now);",
    );
    expect(workspaceStoreSource).toContain("applyIncidentReplayStep: (stepId, stepNow?) =>");
  });
});
