...
describe("phaseOneWorkspaceRouteChecklist", () => {
  ...
  it("defines operations proof moments without staged demo wording", () => {
    const serialized = JSON.stringify(phaseOneWorkspaceRouteChecklist);
    ...
    for (const entry of phaseOneWorkspaceRouteChecklist) {
  ...
  it("keeps clinic detail proof aligned with the showcase capture route", () => {
    const clinicDetail = phaseOneWorkspaceRouteChecklist.find((entry) =>
