...
import { workspaceAlerts } from "@/lib/workspace/alerts";
import { getFreshnessFromTimestamp } from "@/lib/workspace/freshness";
import { walkthroughLeads } from "@/lib/workspace/leads";
import { OPERATIONS_INCIDENT } from "@/lib/workspace/operations-scenario";
import { workspaceAuditEvents, workspaceClinicStates, workspaceReports } from "@/lib/workspace/reports";
import type {
  Alert,
  AuditEvent,
  ClinicCurrentState,
...
export function createInitialWorkspaceState(): WorkspaceState {
  return {
    province: DISTRICT_OPERATIONS_PROVINCE,
    district: DISTRICT_OPERATIONS_DISTRICT,
    clinics: workspaceClinics.map((clinic) => ({ ...clinic })),
    clinicStates: workspaceClinicStates.map((state) => ({ ...state })),
    reports: workspaceReports.map((report) => ({ ...report })),
    alerts: workspaceAlerts.map((alert) => ({ ...alert })),
    auditEvents: workspaceAuditEvents.map((event) => ({ ...event })),
    leads: walkthroughLeads.map((lead) => ({ ...lead })),
    role: "founder_admin",
    offlineQueue: [],
    lastSyncAt: null,
  };
