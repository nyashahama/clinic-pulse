import { getWorkspaceImage } from "@/lib/workspace/images";
import type {
  Alert,
  Clinic,
  ClinicCurrentState,
...
export function getStatusCounts(state: WorkspaceState): Record<ClinicStatus, number> {
...
export function getClinicRows(state: WorkspaceState): ClinicRow[] {
  return state.clinics
    .map((clinic) => {
      ...
      return {
        ...clinic,
        ...clinicState,
        image: getWorkspaceImage(clinic.imageKey),
      };
    })
...
export function getActiveAlerts(state: WorkspaceState): Alert[] {
  return state.alerts
...
export function getClinicReports(state: WorkspaceState, clinicId: string): ReportEvent[] {
  return state.reports
    .filter((report) => report.clinicId === clinicId)
    .sort((left, right) => new Date(right.receivedAt).getTime() - new Date(left.receivedAt).getTime());
}

export function getClinicAuditEvents(state: WorkspaceState, clinicId: string) {
...
export function getRecentReportStream(state: WorkspaceState): ReportStreamItem[] {
  return state.reports
    .map((report) => {
