...
import type { AuditEvent, ClinicRow, QueuedOfflineReport, ReportStreamItem } from "@/lib/workspace/types";
...
describe("ClinicPulse API mappers", () => {
  it("maps backend hydration payloads into the stable workspace state shape", () => {
    ...
    const state = mapApiWorkspaceHydrationToState(
      {
  ...
  it("maps backend clinic detail and status to ClinicRow-compatible data", () => {
    ...
    expect(row).toEqual({
      ...
      stockPressure: "low",
      queuePressure: "high",
      image: getWorkspaceImage("clinic-front-02"),
    } satisfies ClinicRow);
  });
