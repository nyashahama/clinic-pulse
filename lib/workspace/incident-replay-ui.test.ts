...
const scenarioControlsPath = path.join(
  process.cwd(),
  "components",
  "workspace",
  "scenario-controls.tsx",
);
const workspacePageClientPath = path.join(
  process.cwd(),
  "app",
  "(workspace)",
  "district",
...
describe("incident replay controls and page wiring source boundary", () => {
  it("contains the replay control and page orchestration integration points", () => {
    const controlsSource = readFileSync(scenarioControlsPath, "utf8");
    const pageSource = readFileSync(workspacePageClientPath, "utf8");
    ...
    expect(pageSource).toContain("resetWorkspace();");
