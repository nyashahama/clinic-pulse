...
it("uses admin clinic detail return sources for back navigation", () => {
  const source = readFileSync("app/(workspace)/demo/clinics/[clinicId]/page-client.tsx", "utf8");
...
it("defines canonical detail routes for entity-backed admin evidence rows", () => {
  ...
  for (const route of routes) {
    ...
    expect(source).toContain('requireDashboardWorkflowAccess("admin")');
...
it("uses operational detail layouts for report and lead details", () => {
  ...
  const demoReportDetail = readFileSync("app/(workspace)/demo/reports/[reportId]/page-client.tsx", "utf8");
...
it("links audit evidence rows to canonical entity details", () => {
  const auditEvidence = readFileSync("app/(workspace)/admin/audit-evidence/page.tsx", "utf8");
...
it("links integration and security entity rows to canonical details", () => {
  const integrations = readFileSync("app/(workspace)/admin/integrations/page.tsx", "utf8");
