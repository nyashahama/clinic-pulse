...
it("links reporting coverage clinic rows to operational clinic detail", () => {
  ...
  expect(modelSource).toContain("buildAdminReportDetailHref");
...
it("uses admin clinic detail return sources for back navigation", () => {
  const source = readFileSync("app/(demo)/district/clinics/[clinicId]/page-client.tsx", "utf8");
...
it("uses operational detail layouts for report and lead details", () => {
  ...
  const demoReportDetail = readFileSync("app/(demo)/district/reports/[reportId]/page-client.tsx", "utf8");
